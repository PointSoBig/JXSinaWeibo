...
#import "LocationServiceRequest.h"

@implementation LocationServiceRequest
...
{
    self.path = @"place/nearby/pois";
    self.parameters = @{@"access_token":[AccountModel getPassword],@"lat":@(lat),@"long":@(longitude),@"range":@(range),@"q":ValidString(q),@"category":ValidString(category),@"count":@(count),@"page":@(page),@"sort":@(sort),@"offset":@(offset)};
...
{
    self.path = @"place/pois/add_checkin";
    self.parameters = @{@"access_token":[AccountModel getPassword],@"poiid":ValidString(poiid),@"status":ValidString(status),@"pic":@"",@"public":@(public)};
